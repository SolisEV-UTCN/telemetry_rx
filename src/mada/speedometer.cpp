...
Speedometer::Speedometer(QQuickItem *parent)
    : QQuickPaintedItem(parent), m_SpeedometerSize(500), m_startAngle(240),
      m_alignAngle(220),                      // should be 360 - 2*m_startAngle;
      m_lowestRange(0), m_highestRange(4000), // max analog val from adc
      m_speed(2430), m_arcWidth(10), m_outerColor(QColor(128, 255, 0)),
      m_innerColor(QColor(51, 88, 155, 80)), m_textColor(QColor(255, 255, 255)),
...
void Speedometer::paint(QPainter *painter) {
  ...
  painter->drawArc(
      rect.adjusted(m_arcWidth, m_arcWidth, -m_arcWidth, -m_arcWidth),
  ...
  painter->setPen(pen);
  painter->drawPie(rect.adjusted(pieSize, pieSize, -pieSize, -pieSize),
                   startAngle * 16, spanAngle * 16);
  painter->restore();
  ...
  painter->setPen(pen);
  painter->drawText(
      rect.adjusted(m_SpeedometerSize / 30, m_SpeedometerSize / 30,
                    -m_SpeedometerSize / 30, -m_SpeedometerSize / 4),
  ...
  pen.setColor(m_outerColor);
  qreal valueToAngle =
      ((m_speed - m_lowestRange) / (m_highestRange - m_lowestRange)) *
      spanAngle;
  painter->setPen(pen);
  painter->drawArc(
      rect.adjusted(m_arcWidth, m_arcWidth, m_arcWidth, m_arcWidth),
      startAngle * 16, valueToAngle * 16);
  painter->restore();
}
